...
private[oap] class VMemCacheManager(sparkEnv: SparkEnv) extends MemoryManager with Logging {
  ...
  private val vmUsefulRation = conf.getDouble(OapConf.OAP_AEP_USEFUL_RATION.key, 1.0)
  private val vmUsefulSize = (vmInitialSize * vmUsefulRation).toLong
  private val lock = new Object
  @volatile private var initialized = false
  logInfo("initialzing vmemcache memory manager")
  private val vmemcacheEnable_ = {
    val dcpmCacheBackend = SparkEnv.get.conf.get(OapConf.OAP_FIBERCACHE_MEMORY_MANAGER).toLowerCase
    dcpmCacheBackend match {
      case "vmemcache" => true
      case "memkind" => false
      case _ => throw new SparkException(s"DCPM Cache backend ${dcpmCacheBackend} " +
        s"is not supported.")
    }
  }

  def isEnableVMEMCache: Boolean = {
    vmemcacheEnable_
  }
  ...
  if (vmemcacheEnable_) initializeVMEMCache()

  // TODO: a config to control max memory size
  private val (_cacheMemory, _cacheGuardianMemory) = {
    ((vmUsefulSize * 0.9).toLong, (vmUsefulSize * 0.1).toLong)
  }
  // TODO: Atomic is really needed?
  private val _memoryUsed = new AtomicLong(0)
  ...
  override def dataCacheMemory: Long = _cacheMemory
  ...
  override private[filecache] def free(block: MemoryBlockHolder): Unit = {
    logInfo("vmemcache doesn't need to handle cache free.")
  }
